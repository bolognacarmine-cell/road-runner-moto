...
class SilktideCookieBanner {
  ...
  init() {
    console.log('Silktide: Inizializzazione del Consent Manager...');
    this.createWrapper();
    ...
    if (this.shouldShowBanner()) {
      console.log('Silktide: Mostro il banner automatico');
      this.createBanner();
      this.showBackdrop();
    } else {
      console.log('Silktide: Banner già accettato, mostro l\'icona');
      this.showCookieIcon();
    }
  ...
  getBannerContent() {
    const bannerDescription = this.config.text?.banner?.description || "<p>Utilizziamo i cookie sul nostro sito per migliorare la tua esperienza utente, fornire contenuti personalizzati e analizzare il nostro traffico.</p>";
    const acceptAllButtonText = this.config.text?.banner?.acceptAllButtonText || 'Accetta tutto';
    const rejectNonEssentialButtonText = this.config.text?.banner?.rejectNonEssentialButtonText || 'Solo essenziali';
    const preferencesButtonText = this.config.text?.banner?.preferencesButtonText || 'Preferenze';
    ...
    return `
      ...
      <div class="actions">
        ...
        <div class="actions-row">
          ...
          <a class="silktide-logo" href="https://silktide.com/consent-manager" target="_blank" rel="noreferrer" aria-label="Visita Silktide Consent Manager">
  ...
  createBanner() {
    ...
    // Assicuriamo la visibilità immediata
    if (this.banner) {
  ...
  getModalContent() {
    const preferencesTitle = this.config.text?.preferences?.title || 'Personalizza le preferenze';
    const preferencesDescription = this.config.text?.preferences?.description || "<p>Rispettiamo la tua privacy. Puoi scegliere di non consentire alcuni tipi di cookie.</p>";
    const acceptAllButtonText = this.config.text?.banner?.acceptAllButtonText || 'Accetta tutto';
    const rejectNonEssentialButtonText = this.config.text?.banner?.rejectNonEssentialButtonText || 'Solo essenziali';
    const creditLinkText = this.config.text?.preferences?.creditLinkText || 'Ottieni questo banner';
      ...
      <header>
        ...
        <button class="modal-close">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M19.4081 3.41559C20.189 2.6347 20.189 1.36655 19.4081 0.585663C18.6272 -0.195221 17.3591 -0.195221 16.5782 0.585663L10 7.17008L3.41559 0.59191C2.6347 -0.188974 1.36655 -0.188974 0.585663 0.59191C-0.195221 1.37279 -0.195221 2.64095 0.585663 3.42183L7.17008 10L0.59191 16.5844C-0.188974 17.3653 -0.188974 18.6335 0.59191 19.4143C1.37279 20.1952 2.64095 20.1952 3.42183 19.4143L10 12.8299L16.5844 19.4081C17.3653 20.189 18.6335 20.189 19.4081 19.4081C20.1952 18.6272 20.1952 17.3591 19.4143 16.5782L12.8299 10L19.4081 3.41559Z" fill="currentColor"/>
          </svg>
        </button>
      </header>
...
window.silktideCookieBannerManager = {
  ...
  toggleModal: function(show) {
    if (this.instance) {
      this.instance.toggleModal(show);
    }
  }
};
